...
handlers.index = (data, callback) => {
  if (data.method === 'get') {
    ...
    let templateData = {
      ...
      'head.description': 'Free uptime monitoring',
      'body.class': 'index'
    };
...
handlers.sessionCreate = (data, callback) => {
  if (data.method === 'get') {
    ...
    let templateData = {
      ...
      'head.description': 'Enter your phone an password',
...
handlers.favicon = (data, callback) => {
  if (data.method === 'get') {
    helpers.getStaticAsses('favicon.ico', (err, data) => {
      if (!err && data) {
...
handlers.public = (data, callback) => {
  if (data.method === 'get') {
    ...
    if (trimmedAssetName) {
      helpers.getStaticAsses(trimmedAssetName, (err, data) => {
        if (!err && data) {
  ...
  if(phone && password){
    ...
    _data.read('users',phone,function(err,userData){
      if(!err && userData){
        ...
        _performance.mark('beginning has password');
        var hashedPassword = helpers.hash(password);
        _performance.mark('hashing password complete');
        if(hashedPassword == userData.hashedPassword){
          ...
          _data.create('tokens',tokenId,tokenObject,function(err){
            if(!err){
              ...
              _performance.measure('Password hashing', 'beginning has password', 'hashing password complete');
        ...
        _data.read('users',userPhone,function(err,userData){
          if(!err && userData){
            ...
            if(userChecks.length < config.maxChecks){
              ...
              const hostName = typeof parseUrl.hostname === 'string' && parseUrl.hostname.length ? parseUrl.hostname : '';
